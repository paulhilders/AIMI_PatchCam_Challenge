...
def getDataSets():
    ...
    validset_y = f['y']
    train_set = [{"x": torch.from_numpy(trainset_x[i]).permute(2, 0, 1).float(), "labels": trainset_y[i]} for i in range(len(trainset_y))]
    test_set = [{"x": torch.from_numpy(testset_x[i]).permute(2, 0, 1).float(), "labels": testset_y[i]} for i in range(len(testset_y))]
    valid_set = [{"x": torch.from_numpy(validset_x[i]).permute(2, 0, 1).float(), "labels": validset_y[i]} for i in range(len(validset_y))]
